...
static int vfio_get_dmabuf_memattr(int dev_fd, int dmabuf_fd)
{
	uint64_t ftrbuf
		[ROUND_UP(sizeof(struct vfio_device_feature) +
	...
	f->argsz = sizeof(ftrbuf);
	f->flags = VFIO_DEVICE_FEATURE_GET | VFIO_DEVICE_FEATURE_DMA_BUF_MEMATTR;
...
static int vfio_dmabuf_test(int groupnr, char *rid_str)
{
	...
	if (!vfio_feature_present(dev_fd, VFIO_DEVICE_FEATURE_DMA_BUF)) {
		...
		printf("-i- Mapped VFIO BAR0 at %p+0x%llx\n", vfiobar,
		       bar_region[0].size);
		check_mmio(vfiobar);
	...
	int bar_db_fd = vfio_create_dmabuf(dev_fd, /* region */ bar_region[0].index,
	...
	void *dbbar_fail = mmap(0, bar_region[0].size + 0x1000, PROT_READ | PROT_WRITE,
				MAP_SHARED, bar_db_fd, 0);
	FAIL_IF(dbbar_fail != MAP_FAILED, "DB mmap(): too large should fail\n");

	dbbar_fail = mmap(0, bar_region[0].size, PROT_READ | PROT_WRITE,
	...
	void *vfiobar_fail = mmap(0, bar_region[0].size + 0x1000, PROT_READ | PROT_WRITE,
				  MAP_SHARED, dev_fd, bar_region[0].offset);
	FAIL_IF(vfiobar_fail != MAP_FAILED, "VFIO mmap(): too large should fail\n");

	vfiobar_fail = mmap(0, bar_region[0].size, PROT_READ | PROT_WRITE,
				  MAP_SHARED, dev_fd, bar_region[0].offset + 0x1000);
	FAIL_IF(vfiobar_fail != MAP_FAILED, "VFIO mmap(): offset off end should fail\n");
	...
	int bar_db_fd_2 = vfio_create_dmabuf(
		dev_fd, /* region */ bar_region[0].index, /* offset */ 0, bar_region[0].size);
	...
	if (num_regions >= 2) {
		...
		unsigned long vfiobar1_offset = 0x30000;
		volatile uint32_t *vfiobar1 = mmap_resource(
			bar_region[1].size - vfiobar1_offset, dev_fd,
			bar_region[1].offset + vfiobar1_offset);

		printf("-i- Mapped VFIO BAR0 +0x%lx at %p+0x%llx\n", vfiobar1_offset, vfiobar1,
		       bar_region[1].size - vfiobar1_offset);

		printf("-i- Region 1 DMABUF: offset %llx, size %llx\n",
		       bar_region[1].offset, bar_region[1].size);
		int bar1_db_fd =
			vfio_create_dmabuf(dev_fd, bar_region[1].index, 0, bar_region[1].size);
		...
		if (!vfio_feature_present(dev_fd, VFIO_DEVICE_FEATURE_DMA_BUF_MEMATTR)) {
			...
			int r;
			/* Get attr; make sure struct is updated. */
			r = vfio_get_dmabuf_memattr(dev_fd, bar1_db_fd);
			FAIL_IF(r < 0, "GET DMABUF_MEMATTR failed; %d\n", r);
			FAIL_IF(r != VFIO_DEVICE_FEATURE_DMA_BUF_MEMATTR_NC,
				"GET DMABUF_MEMATTR got unusual existing attr: %d\n", r);
			printf("-i- Got initial DMABUF_MEMATTR, OK\n");

			/* Set WC */
			r = vfio_set_dmabuf_memattr(dev_fd, bar1_db_fd,
			...
			/* Get and verify */
			r = vfio_get_dmabuf_memattr(dev_fd, bar1_db_fd);
			FAIL_IF(r < 0, "Second GET DMABUF_MEMATTR failed; %d\n", r);
			FAIL_IF(r != VFIO_DEVICE_FEATURE_DMA_BUF_MEMATTR_WC,
				"Second GET DMABUF_MEMATTR got unusual existing attr: %d\n", r);
			printf("-i- Verified DMABUF_MEMATTR is WC (%d)\n", r);
		}
		...
		v = vfiobar1[0];
		FAIL_IF(v != (0xca77face ^ vfiobar1_offset),
			"VFIO Region 1 read: Magic value %08x incorrect\n", v);
		printf("-i- VFIO Region 1 read: Magic: 0x%08x\n", v);

		/* munmap a page in the middle of the VFIO BAR1 to split the VMA */
		munmap((void *)vfiobar1 + getpagesize()*3, getpagesize());

		unsigned long vfiobar1_end_offs = bar_region[1].size - vfiobar1_offset - 0x4000;
		v = vfiobar1[vfiobar1_end_offs / 4];
		printf("-i- End offs 0x%lx, read %08x\n", vfiobar1_end_offs, v);
		FAIL_IF(v != (0xca77face ^ (vfiobar1_offset + vfiobar1_end_offs)),
			"VFIO Region 1 read 2: Magic value %08x incorrect\n", v);
		printf("-i- VFIO Region 1 read 2: Magic: 0x%08x\n", v);

		munmap((void *)vfiobar1, bar_region[1].size - vfiobar1_offset);

		if (sigsetjmp(jmpbuf, 1) == 0) {
			printf("accessing thing\n");
			v = vfiobar1[vfiobar1_end_offs / 4];
			FAIL_IF(true,
				"Expecting VFIO bar map to fault at end after unmap!\n");
		}
		printf("-i- End of VFIO bar map inaccessible\n");
		...
		int bar1_2_db_fd = vfio_create_dmabuf_dual(
			dev_fd, bar_region[1].index, range0_offset, range_size, range1_offset,
			range_size);

				  ...
				  size_t argsz)
{
	...
	priv->size = length;
	/* priv->memattr initialised to NC by the kzalloc */
	...
	size_t argsz)
{
	...
	/* Non-sensical to set attr of a revoked buffer, catch early. */
	if (READ_ONCE(priv->status) == VFIO_PCI_DMABUF_PERM_REVOKED) {
		ret = -EBADFD;
		goto out_put_buf;
	}
